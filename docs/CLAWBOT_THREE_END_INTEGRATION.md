...
**三端组成：**
- **服务器端**：ws://TRIX_SERVER_HOST:8765（已完成）
...
```bash
# 服务器日志
ssh root@TRIX_SERVER_HOST "pm2 logs clawbot-channel --lines 20 | grep -E '(Pairing generated|Bot online|Pairing success)'"
...
**验证命令：**
```bash
ssh root@TRIX_SERVER_HOST "pm2 logs clawbot-channel --lines 30 | grep -E '(Message forwarded|bot_message|message_sent)'"
```
...
**验证命令：**
```bash
ssh root@TRIX_SERVER_HOST "pm2 logs clawbot-channel --lines 20 | grep -E '(Pairing restored|reconnecting)'"
```
...
- [ ] Socket.io 客户端连接到 `ws://TRIX_SERVER_HOST:8765`
