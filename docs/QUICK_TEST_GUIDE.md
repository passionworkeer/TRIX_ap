...
```bash
# 方式 1: 使用脚本 (推荐)
ssh root@47.243.55.130 "bash -s" < scripts/restart_cloud_server.sh
...
```bash
ssh root@47.243.55.130 "ps aux | grep cloud_server_advanced && tail -5 /tmp/cloud_server_advanced.log"
```
...
```bash
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db 'SELECT code, user_id, status FROM pairings;'"
```
...
**解决**:
```bash
ssh root@47.243.55.130 "pkill -f cloud_server && nohup python3 /opt/nanobot-cloud/cloud_server_advanced.py > /tmp/cloud_server_advanced.log 2>&1 &"
```
...
**解决**:
```bash
ssh root@47.243.55.130 "lsof -ti:8765 | xargs kill -9"
```
...
# 1. 服务器进程
echo "1️⃣ 服务器进程:"
ssh root@47.243.55.130 "ps aux | grep cloud_server_advanced | grep -v grep"
echo ""

# 2. 端口监听
echo "2️⃣ 端口监听:"
ssh root@47.243.55.130 "lsof -i :8765 | grep LISTEN"
echo ""
...
ssh root@47.243.55.130 "tail -5 /tmp/cloud_server_advanced.log"
echo ""
...
```bash
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db <<EOF
.mode column
.headers on
SELECT
    code AS '配对码',
...
```bash
ssh root@47.243.55.130 "tail -f /tmp/cloud_server_advanced.log | grep --color -E '(配对成功|消息转发|回复转发|User:)'"
```
