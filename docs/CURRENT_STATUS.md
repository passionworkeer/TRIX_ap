...
**已更新**:
- ✅ 云端服务器 URL: `ws://TRIX_SERVER_HOST:8765`
- ✅ WebSocket 连接代码已实现
- ✅ 配对码生成功能已实现
...
**症状**:
```
$ curl -v http://TRIX_SERVER_HOST:8765/
...
* connect to TRIX_SERVER_HOST port 8765 from 0.0.0.0 port 64759 failed: Timed out
* Failed to connect to TRIX_SERVER_HOST port 8765 after 21057 ms: Could not connect to server
```
...
```bash
ssh root@TRIX_SERVER_HOST "curl -s http://100.100.100.200/latest/meta-data/security-groups"
```
...
```bash
# Windows PowerShell
Test-NetConnection -ComputerName TRIX_SERVER_HOST -Port 8765
...
**检查数据库**:
```bash
ssh root@TRIX_SERVER_HOST "sqlite3 /tmp/nanobot.db \"SELECT code, status FROM pairings WHERE code='A1B2C3D4';\""
```
...
# 端口监听
ssh root@TRIX_SERVER_HOST "netstat -tlnp | grep 8765"
...
- **域名配置**: 将 IP TRIX_SERVER_HOST 绑定域名
