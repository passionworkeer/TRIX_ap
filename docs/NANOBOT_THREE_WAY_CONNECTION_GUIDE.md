...
│  ┌──────────────┐     WebSocket      ┌──────────────┐   │
│  │ 本地 Nanobot  │◄──────────────────►│  云端服务器    │   │
│  │ Windows 本地  │    ws://47.243.55.130│  47.243.55.130  │   │
│  │ :5000        │                   │  :8765        │   │
│  └──────────────┘                   └──────┬───────┘   │
│                                              │             │
│                                              │ WebSocket │
...
| 配置项 | 值 |
|--------|-----|
| **服务器 IP** | `47.243.55.130` |
| **WebSocket 端口** | `8765` |
| **WebSocket 路径** | `/nanobot/ws` |
| **完整地址** | `ws://47.243.55.130:8765/nanobot/ws` |
...
| **WebSocket 地址** | `ws://47.243.55.130:8765/nanobot/ws` |
...
**找到第 18 行**：
```python
CLOUD_SERVER = 'ws://47.243.55.130/nanobot/ws'  # WebSocket地址
```

**确认配置正确**：
- ✅ IP 地址正确：`47.243.55.130`
- ✅ 端口正确：`8765`
- ✅ 路径正确：`/nanobot/ws`
- ✅ 协议正确：`ws://`
...
**检查连接日志**：
```
正在连接云端: ws://47.243.55.130:8765/nanobot/ws...
...
1. ✅ 云端服务器是否运行？
   ```bash
   ssh root@47.243.55.130 "ps aux | grep cloud_server"
   ```

2. ✅ 防火墙是否开放 8765 端口？
   ```bash
   ssh root@47.243.55.130 "ufw status | grep 8765"
   ```

3. ✅ 本地配置文件地址是否正确？
   ```python
   # e:\desktop\nanobot\nanobot\web_interface_final.py L18
   CLOUD_SERVER = 'ws://47.243.55.130:8765/nanobot/ws'
   ```
...
2. ✅ 环境变量是否正确？
   ...
   this.serverUrl = 'ws://47.243.55.130:8765';
   ```
3. ✅ 浏览器控制台错误？
   ```
   按 F12 打开控制台查看错误信息
...
4. ✅ WebSocket 连接地址？
   ```javascript
   // 应该是
   ws://47.243.55.130:8765
   // 不是
   ws://47.243.55.130/nanobot/ws  (注意路径)
   ```
...
**云端服务器日志**：
```bash
ssh root@47.243.55.130 "journalctl -u cloud-server | grep '配对码注册'"
```
...
1. **启动云端服务器**（如果没有运行）：
   ```bash
   ssh root@47.243.55.130 "systemctl start cloud-server"
   ```
