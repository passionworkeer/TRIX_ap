...
**预期行为：**
1. Clawbot 连接到 `ws://47.243.55.130:8765`
...
**预期行为：**
1. App 连接到 `ws://47.243.55.130:8765`
...
**验证命令：**
```bash
ssh root@47.243.55.130 "pm2 logs clawbot-channel --lines 20 --nostream | \
...
**验证命令：**
```bash
ssh root@47.243.55.130 "pm2 logs clawbot-channel --lines 30 --nostream | \
...
**解决方案：**
```bash
ssh root@47.243.55.130 "pm2 restart clawbot-channel && sleep 3 && pm2 logs clawbot-channel --lines 10"
```
