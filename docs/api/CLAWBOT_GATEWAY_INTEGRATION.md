...
> **核心逻辑**：**电脑端 Gateway 生成二维码 → 手机 App 扫描配对**
...
**流程说明**：
1. **用户在电脑端启动 Clawbot Gateway**
2. **Gateway 生成配对二维码**（包含 Gateway URL + 临时配对 Token）
3. **用户打开手机 App，扫描二维码**
4. **手机解析二维码，发送配对请求到 Gateway**
5. **Gateway 在电脑端显示配对确认**（显示设备名称、类型等）
6. **用户在电脑端点击"允许"**
7. **Gateway 返回设备专用 Token**
8. **手机使用 Token 建立 WebSocket 长连接**

### 方式一：直接 WebSocket 连接（当前使用）
