...
- `NanobotContext` 在 `NanobotPairing` 页面使用，若用户之前保存了配对码，也会**自动重连** `ws://TRIX_SERVER_HOST:8766`。  
...
| 端 | URL |
|----|-----|
| Python 后端（`web_interface_final.py`） | `ws://TRIX_SERVER_HOST:8765` |
| 前端 App（`NanobotBridge.ts` 默认值） | `ws://TRIX_SERVER_HOST:8766` |
...
```bash
wscat -c ws://TRIX_SERVER_HOST:8766
```
