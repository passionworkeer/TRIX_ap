...
**浏览器控制台 (F12) 预期输出**:
```
[NanobotBridge] 服务器地址: ws://47.243.55.130:8765
...
**云端日志检查**:
```bash
ssh root@47.243.55.130 "tail -30 /tmp/cloud_server_advanced.log"
```
...
**数据库验证**:
```bash
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db 'SELECT code, nanobot_device_id, app_device_id, user_id, status FROM pairings;'"
```
...
2. ✅ 防火墙是否开放 8765 端口?
   ```bash
   ssh root@47.243.55.130 "iptables -L -n | grep 8765"
   ```
...
# 查看最近 50 行
ssh root@47.243.55.130 "tail -50 /tmp/cloud_server_advanced.log"

# 搜索特定日志
ssh root@47.243.55.130 "grep '配对成功' /tmp/cloud_server_advanced.log"
```
...
```bash
# 查看所有配对关系
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db 'SELECT * FROM pairings;'"

# 查看活跃配对
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db \"SELECT code, status, created_at FROM pairings WHERE status='connected';\""

# 查看用户绑定
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db 'SELECT * FROM user_bindings;'"

# 清理过期配对码
ssh root@47.243.55.130 "sqlite3 /tmp/nanobot.db \"DELETE FROM pairings WHERE status='waiting' AND datetime(expires_at) < datetime('now');\""
```
...
```bash
# CPU 和内存使用
ssh root@47.243.55.130 "top -n 1 | grep python"

# 磁盘使用
ssh root@47.243.55.130 "df -h"

# 网络连接
ssh root@47.243.55.130 "netstat -an | grep 8765"
```
...
```bash
# 停止服务器
ssh root@47.243.55.130 "pkill -f cloud_server_advanced.py"

# 启动服务器
ssh root@47.243.55.130 "nohup python3 /opt/nanobot-cloud/cloud_server_advanced.py > /tmp/cloud_server_advanced.log 2>&1 &"

# 一键重启
ssh root@47.243.55.130 "pkill -f cloud_server_advanced.py && sleep 2 && nohup python3 /opt/nanobot-cloud/cloud_server_advanced.py > /tmp/cloud_server_advanced.log 2>&1 &"
```
...
```bash
# 优雅停止
ssh root@47.243.55.130 "pkill -15 -f cloud_server_advanced.py"

# 强制停止
ssh root@47.243.55.130 "pkill -9 -f cloud_server_advanced.py"

# 使用 PID
ssh root@47.243.55.130 "kill $(cat /tmp/cloud_server_advanced.pid)"
```
