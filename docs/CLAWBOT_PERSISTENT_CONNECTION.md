...
**查看当前连接的 Clawbot**：
```bash
ssh root@47.243.55.130 "pm2 logs clawbot-channel --lines 200 | grep 'Total bots'"
```

**查看配对记录**：
```bash
ssh root@47.243.55.130 "sqlite3 /opt/clawbot-channel/data/pairing.db 'SELECT device_id, user_id, status FROM pairings WHERE status=\"paired\"'"
```
