...
- [x] **T1.9.1** 创建 `src/screens/PointsMall.test.tsx` 文件
- [x] **T1.9.2** 测试页面渲染
- [x] **T1.9.3** 测试分类筛选
- [x] **T1.9.4** 测试商品列表渲染
- [x] **T1.9.5** 测试兑换按钮点击
...
- [x] **T1.11.1** 验证积分扣减双重检查
- [x] **T1.11.2** 验证请求防重放 (nonce)
- [x] **T1.11.3** 验证商品数据来源可信
- [x] **T1.11.4** 验证错误信息不泄露敏感数据
- [x] **T1.11.5** 验证 HTTPS 传输
...
- [x] **T3.4.1** 实现定时刷新好友位置 (每 30 秒)
- [x] **T3.4.2** 实现位置变化动画
- [x] **T3.4.3** 添加位置更新 indicator

#### 3.5 位置共享设置 (1h)

- [x] **T3.5.1** 在设置页面添加位置共享入口
- [x] **T3.5.2** 实现位置共享开关
- [x] **T3.5.3** 实现可见范围设置 (所有人/仅好友)
...
- [x] T1.9.1 创建 PointsMall.test.tsx
- [x] T1.9.2 测试渲染
- [x] T1.9.3 测试筛选
- [x] T1.9.4 测试列表
- [x] T1.9.5 测试兑换
