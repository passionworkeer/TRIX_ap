...
3. **手动上传（备选）**
   ...
   # 使用 sftp 上传
   lftp -u root -p $PASSWORD TRIX_SERVER_HOST << EOF
...
2. **下载文件（备份）**
   ```bash
   scp root@TRIX_SERVER_HOST:/opt/clawbot-channel/server.js .
   scp root@TRIX_SERVER_HOST:/opt/clawbot-channel/config/database.js .
   ```
...
4. **上传文件**
   ```bash
   scp server.js root@TRIX_SERVER_HOST:/opt/clawbot-channel/
   scp config/database.js root@TRIX_SERVER_HOST:/opt/clawbot-channel/config/
   ```

5. **重启服务**
   ```bash
   ssh root@TRIX_SERVER_HOST 'pm2 restart clawbot-channel'
   ```
