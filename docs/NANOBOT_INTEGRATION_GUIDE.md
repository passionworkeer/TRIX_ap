...
| 端 | 状态 | 地址 |
|----|------|------|
| 云端服务器 | ✅ 运行中 | `ws://47.243.55.130:8765` |
...
| 云端地址 | `ws://47.243.55.130:8765/nanobot/ws` (L18) |
...
| 云端地址 | `ws://47.243.55.130:8765` (L49) |
...
# 检查端口
ssh root@47.243.55.130 "netstat -tuln | grep 8765"
```
...
**云端日志**:
```bash
ssh root@47.243.55.130 "tail -f /tmp/cloud_server.log"
```
...
**检查清单**:
1. ✅ 云端服务器是否运行？`ssh root@47.243.55.130 "ps aux | grep cloud_server"`
2. ✅ 防火墙是否开放端口？
3. ✅ 配置文件地址是否正确？
...
- **云端日志**: `ssh root@47.243.55.130 "journalctl -u cloud-server | grep '配对码'"`
