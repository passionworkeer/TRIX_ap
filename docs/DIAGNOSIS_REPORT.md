...
```bash
ssh root@TRIX_SERVER_HOST "sqlite3 /opt/clawbot-channel/data/pairing.db 'DELETE FROM pairings WHERE status=\"paired\" AND user_id IS NULL'"
```
...
5. **验证数据库**
   ```bash
   ssh root@TRIX_SERVER_HOST "sqlite3 /opt/clawbot-channel/data/pairing.db 'SELECT pairing_code, device_id, user_id, status FROM pairings ORDER BY created_at DESC LIMIT 1'"
   ```
