...
**复制公钥到服务器**:
```bash
ssh-copy-id -i ~/.ssh/github_deploy.pub root@47.243.55.130
```
...
```bash
# 方式 A：使用 SCP
scp trix-build.tar.gz root@47.243.55.130:/var/www/trix-3d-companion-web/

# 方式 B：使用 rsync
rsync -avz --delete dist/ root@47.243.55.130:/var/www/trix-3d-companion-web/
```
...
3. 验证 SSH 连接：`ssh -i ~/.ssh/github_deploy root@47.243.55.130`
