...
> **目标**: 通过扫码方式接入 Clawbot（**电脑生成二维码，手机扫描**），实现稳定的长连接通信
...
> **重要**: 配对流程为 **电脑端 Gateway 生成二维码 → 手机 App 扫描配对**
...
```json
{
  "gateway": {
    ...
    "auth": {
      "mode": "token",
      "token": "trix-master-token-change-me"
    }
  },
  "channels": {
    "webchat": {
...
- **完整版**: 使用扫码功能扫描 Gateway 生成的二维码
- **简化版**: 手动输入 Gateway 地址和 Token
