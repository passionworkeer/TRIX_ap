...
|------|------|
| **SSH 用户** | `root` |
| **SSH 主机** | `47.243.55.130` |
| **SSH 端口** | `22` |
| **连接命令** | `ssh root@47.243.55.130` |
...
# 跳过主机密钥检查（自动化脚本）
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@47.243.55.130
```
...
1. 在 Nginx 配置中修改：
   ```nginx
   server_name your-domain.com;  # 原来是 47.243.55.130
   ```

2. 配置 DNS 解析指向 `47.243.55.130`
