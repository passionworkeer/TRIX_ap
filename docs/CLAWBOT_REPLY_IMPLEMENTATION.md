...
SERVER_URL = 'ws://TRIX_SERVER_HOST:8765'
...
**接口地址**: `http://TRIX_SERVER_HOST:8765/webhook/clawbot`
...
# 配置
WEBHOOK_URL = 'http://TRIX_SERVER_HOST:8765/webhook/clawbot'
...
1. **检查 Clawbot 是否已配对**：
   ```bash
   ssh root@TRIX_SERVER_HOST "sqlite3 /opt/clawbot-channel/data/pairing.db 'SELECT device_id, user_id, status FROM pairings WHERE device_id=\"clawbot-001\"'"
   ```
...
```bash
ssh root@TRIX_SERVER_HOST "sqlite3 /opt/clawbot-channel/data/pairing.db 'SELECT * FROM pairings WHERE status=\"paired\"'"
```

### 查看消息记录

```bash
ssh root@TRIX_SERVER_HOST "sqlite3 /opt/clawbot-channel/data/pairing.db 'SELECT * FROM messages ORDER BY created_at DESC LIMIT 10'"
```
