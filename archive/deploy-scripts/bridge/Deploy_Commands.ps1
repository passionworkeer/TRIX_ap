...
scp relay_server.py root@TRIX_SERVER_HOST:/root/nanobot/

# 4. 在服务器上执行部署
Write-Host "正在部署..." -ForegroundColor Yellow
ssh root@TRIX_SERVER_HOST "cd /root/nanobot && pip3 install websockets aiohttp -q && pm2 stop relay-server 2>/dev/null; pm2 delete relay-server 2>/dev/null; pm2 start relay_server.py --name relay-server --interpreter python3 && pm2 save && pm2 status relay-server"
