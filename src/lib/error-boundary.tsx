...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  override componentDidCatch(error: Error, errorInfo: ErrorInfo): void {
    ...
    // 记录错误到日志
    logger.ui.error('ErrorBoundary caught an error:', {
      code: appError.code,
  ...
  private getErrorMessage(): string {
    ...
    return '抱歉，应用遇到了意外错误。我们已经记录了这个问题，请尝试刷新页面或返回首页。';
  }
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-red-50 via-white to-orange-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 text-center">
          ...
          {/* 开发环境显示错误详情 */}
          {isDev() && this.props.showDetails !== false && error && (
 ...
 * @param errorBoundaryProps 错误边界 props
 */
...
export function to<T>(
  ...
  return promise
    ...
    .catch<[Error, null]>((error) => [error, null]);
}
 ...
 * 使用方式：
 * ```tsx
 * const { error, handleAsync } = useAsyncErrorBoundary();
 *
 * const loadData = async () => {
 *   const [err, data] = await handleAsync(fetchData());
 *   if (err) {
 *     // 处理错误
 *   }
 * };
