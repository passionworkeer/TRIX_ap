...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen, onClose }) => {
  ...
  const handleMarkAllAsRead = async () => {
    // 好友请求必须通过“接受/拒绝”处理，避免一键已读导致审批丢失
        ...
        <div className="flex-1 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            notifications.map(notification => {
              const displayContent = getNotificationDisplayContent(notification.content);
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-600 mb-2 line-clamp-2">{displayContent}</p>
                      <span className="text-xs text-slate-400">{formatTimestamp(notification.created_at)}</span>
                    </div>

                    {notification.avatar_url && (
                      <Avatar
                        name={displayContent.split(' ')[0] || 'User'}
