...
3. **服务器文件权限错误**
   ```bash
   # 修复权限
   ssh root@47.243.55.130 "chown -R www-data:www-data /var/www/html/"
   ```
...
# 查看 Nginx 日志
ssh root@47.243.55.130 "tail -f /var/log/nginx/access.log"
```

## 📱 验证部署

部署完成后，访问 http://47.243.55.130 验证：
...
# 3. 单独重启后端
ssh root@47.243.55.130 "cd /opt/clawbot-channel && pm2 restart clawbot-channel"
```
