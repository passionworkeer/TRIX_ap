...
echo [2/4] 上传公钥到服务器...
type %USERPROFILE%\.ssh\github_actions.pub | ssh root@TRIX_SERVER_HOST "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
echo ✅ 公钥已上传到服务器
echo.
...
echo    Value: https://__SUPABASE_PROJECT_REF_REDACTED__.supabase.co
echo.
echo    Name: VITE_SUPABASE_ANON_KEY
echo    Value: __SUPABASE_ANON_KEY_REDACTED__
echo.
echo 3. 添加完成后，按任意键继续...
pause > nul
echo.
