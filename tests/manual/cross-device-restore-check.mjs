...
async function clearNativeSession(page) {
  await page.evaluate(() => {
    localStorage.removeItem('trix_native_channel_sessions_v2');
  });
}
...
try {
  ...
  await clearNativeSession(desktopPage);
  ...
  await clearNativeSession(mobilePage);
