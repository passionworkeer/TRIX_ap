...
```bash
# 上传 Relay Server
scp e:\desktop\trix-3d-companion\bridge\relay_server_with_bridge.py root@TRIX_SERVER_HOST:/root/nanobot/

# 上传部署脚本
scp e:\desktop\trix-3d-companion\bridge\deploy_production.sh root@TRIX_SERVER_HOST:/root/nanobot/
```
...
**预期输出**:
```
[Bridge] 🚀 正在连接云端 Relay Server: ws://TRIX_SERVER_HOST:8765
...
# 3. 检查防火墙
ssh root@TRIX_SERVER_HOST "sudo ufw status"

# 4. 开放端口
ssh root@TRIX_SERVER_HOST "sudo ufw allow 8765"
```
...
# 实时日志
ssh root@TRIX_SERVER_HOST "pm2 logs relay-server"

# 资源监控
ssh root@TRIX_SERVER_HOST "pm2 monit"
```
