...
**服务器**:
```bash
ssh root@47.243.55.130 "pm2 status relay-server"
```
...
2. 如果未运行，启动它
   ```bash
   ssh root@47.243.55.130 "cd /root/nanobot && pm2 start relay_server.py --name relay-server --interpreter python3"
   ```

3. 检查防火墙
   ```bash
   ssh root@47.243.55.130 "sudo ufw allow 8765/tcp"
   ```
...
2. 重启Web服务器
   ```bash
   ssh root@47.243.55.130 "systemctl restart nginx"
   ```
...
- [ ] **云端Relay Server运行中**
  - 命令: `ssh root@47.243.55.130 "pm2 status relay-server"`
  - 结果: 显示relay-server状态为online

- [ ] **手机可以访问服务器**
  - 浏览器打开: `http://47.243.55.130`
  - 结果: 可以看到App界面
